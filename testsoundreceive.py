...
# Title: Test sound receive (mono channel layout)
...
class testsoundreceive(gr.top_block, Qt.QWidget):

    def __init__(self, wav_out='received.wav', zmqsource='tcp://127.0.0.1:13535'):
        gr.top_block.__init__(self, "Test sound receive (mono channel layout)", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Test sound receive (mono channel layout)")
        ...
        self.settings = Qt.QSettings("GNU Radio", "testsoundreceive")
...
def argument_parser():
    ...
    parser.add_argument(
        "--wav-out", dest="wav_out", type=str, default='received.wav',
        help="Set Waveform file output [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=testsoundreceive, options=None):
    if options is None:
        options = argument_parser().parse_args()

    qapp = Qt.QApplication(sys.argv)

    tb = top_block_cls(wav_out=options.wav_out, zmqsource=options.zmqsource)
