...
class test_packet_mux_and_demux(gr.top_block):
    ...
    def __init__(self, in_file='in.bin', out='out.bin', packet_size=64, read_throttle=1024):
        ...
        self.blocks_throttle2_0 = blocks.throttle( gr.sizeof_char*1, read_throttle, True, 0 if "auto" == "auto" else max( int(float(0.1) * read_throttle) if "auto" == "time" else int(0.1), 1) )
        ...
        self.connect((self.blocks_float_to_char_0, 0), (self.blocks_throttle2_0, 0))
    ...
    def set_out(self, out):
        ...
        self.blocks_file_sink_1.open(self.out)
    ...
    def get_read_throttle(self):
        return self.read_throttle

    def set_read_throttle(self, read_throttle):
        self.read_throttle = read_throttle
        self.blocks_throttle2_0.set_sample_rate(self.read_throttle)
...
def argument_parser():
    ...
    parser.add_argument(
        "--read-throttle", dest="read_throttle", type=intx, default=1024,
        help="Set Read throttle (bytes per second) [default=%(default)r]")
    return parser
...
def main(top_block_cls=test_packet_mux_and_demux, options=None):
    ...
    tb = top_block_cls(in_file=options.in_file, out=options.out, packet_size=options.packet_size, read_throttle=options.read_throttle)
