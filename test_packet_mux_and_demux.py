...
class test_packet_mux_and_demux(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, dest_host='127.0.0.1', dest_port='42028', filtersize=84, in_file='in.bin', noisevoltage=0.1, num_pream_packets=10, out='out.bin', packet_size=16, passband_fc=1800, samp_rate=48000, sps=72, txgain=1):
...
def main(top_block_cls=test_packet_mux_and_demux, options=None):
    ...
    tb = top_block_cls(dest_host=options.dest_host, dest_port=options.dest_port, filtersize=options.filtersize, in_file=options.in_file, noisevoltage=options.noisevoltage, num_pream_packets=options.num_pream_packets, out=options.out, packet_size=options.packet_size, passband_fc=options.passband_fc, samp_rate=options.samp_rate, sps=options.sps, txgain=options.txgain)
