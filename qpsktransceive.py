...
class qpsktransceive(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, dest_host='127.0.0.1', dest_port='42028', filtersize=8, in_file="in.bin", noisevoltage=0.1, out_file="out.bin", pktsize=32, sps=64, txgain=1, wav_out='out.wav'):
        ...
        self.blocks_stream_to_tagged_stream_0 = blocks.stream_to_tagged_stream(gr.sizeof_char, 1, pktsize, "packet_len")
        ...
        self.connect((self.blocks_multiply_const_vxx_1, 0), (self.blocks_multiply_xx_0_0_0, 1))
        self.connect((self.blocks_multiply_const_vxx_1, 0), (self.blocks_multiply_xx_0_1, 1))
    ...
    def set_pktsize(self, pktsize):
        self.pktsize = pktsize
        self.blocks_stream_to_tagged_stream_0.set_packet_len(self.pktsize)
        self.blocks_stream_to_tagged_stream_0.set_packet_len_pmt(self.pktsize)
    ...
    def set_txgain(self, txgain):
        self.txgain = txgain
        self.blocks_multiply_const_vxx_1.set_k((2*self.txgain))
...
def main(top_block_cls=qpsktransceive, options=None):
    ...
    tb = top_block_cls(dest_host=options.dest_host, dest_port=options.dest_port, filtersize=options.filtersize, in_file=options.in_file, noisevoltage=options.noisevoltage, out_file=options.out_file, pktsize=options.pktsize, sps=options.sps, txgain=options.txgain, wav_out=options.wav_out)
