...
class test_bpsk_receive(gr.top_block, Qt.QWidget):

    def __init__(self, dest_host='127.0.0.1', dest_port='42028', filtersize=84, noisevoltage=0.1, num_pream_packets=20, out='out.bin', packet_size=16, passband_fc=3600, samp_rate=48000, sps=72, txgain=2, wav_in='record.wav', wav_out='out.wav'):
        gr.top_block.__init__(self, "Test Receving", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Test Receving")
        ...
        self.settings = Qt.QSettings("GNU Radio", "test_bpsk_receive")
...
def main(top_block_cls=test_bpsk_receive, options=None):
    if options is None:
        options = argument_parser().parse_args()

    qapp = Qt.QApplication(sys.argv)

    tb = top_block_cls(dest_host=options.dest_host, dest_port=options.dest_port, filtersize=options.filtersize, noisevoltage=options.noisevoltage, num_pream_packets=options.num_pream_packets, out=options.out, packet_size=options.packet_size, passband_fc=options.passband_fc, samp_rate=options.samp_rate, sps=options.sps, txgain=options.txgain, wav_in=options.wav_in, wav_out=options.wav_out)
